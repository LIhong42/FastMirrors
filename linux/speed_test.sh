...
# 显示帮助信息
show_help() {
    echo -e "${BOLD}Linux 软件源测速脚本${PLAIN}"
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项:"
...
get_test_url() {
    ...
    case "${SYSTEM_ID}" in
        ...
        centos|rhel|rocky|almalinux|fedora)
            ...
            ;;
        arch|manjaro)
            echo "https://${mirror_url}/archlinux/"
            ;;
        alpine)
            echo "https://${mirror_url}/alpine/"
            ;;
        *)
            echo "https://${mirror_url}/"
            ;;
...
test_single_mirror() {
    local mirror_url="$1"
    local test_url=$(get_test_url "$mirror_url")
    local total_time=0
...
run_speed_test() {
    echo -e "${BOLD}========================================${PLAIN}"
    echo -e "${BOLD}    Linux 软件源镜像测速工具${PLAIN}"
    echo -e "${BOLD}========================================${PLAIN}"
    echo ""
    ...
    for ((i=0; i<count; i++)); do
        ...
        if [[ "$avg_time" == "999999" ]]; then
            ...
            results+=("${avg_time}|${mirror}|${desc}|${sys}")
        else
