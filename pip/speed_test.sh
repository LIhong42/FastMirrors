...
MIRRORS_FILE="${PROJECT_ROOT}/mirror-sources/pip.txt"
...
test_single_mirror() {
    local mirror_url="$1"
    local test_url="https://${mirror_url}"
    local total_time=0
...
run_speed_test() {
    ...
    # 显示排序结果
    printf "${BOLD}%-6s %-50s %12s %s${PLAIN}\n" "排名" "镜像地址" "延迟" "描述"
    echo "--------------------------------------------------------------------------------"
    ...
    for result in "${sorted[@]}"; do
        ...
        if [[ "$time" == "999999" ]]; then
            printf "${RED}%-6s %-50s %12s %s${PLAIN}\n" "-" "$mirror" "不可用" "$desc"
            # 不添加不可用的镜像到结果中
        else
            printf "${GREEN}%-6s %-50s %12s %s${PLAIN}\n" "$rank" "$mirror" "${time}ms" "$desc"
            valid_results+=("${mirror}|${desc}|${host}")
